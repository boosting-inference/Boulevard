"""Dropout Boulevard booster built on sklearn histogram-tree internals."""
...
class DropoutBooster(HistGradientBoostingRegressor):
    """Dropout Boulevard booster with BRAT-D residual construction.
    ...
    loop with Boulevard/BRAT-D dropout residual construction.
    """
    ...
    def fit(
        ...
        """Fit the dropout Boulevard booster."""
    ...
    def _check_histogram_backend() -> None:
        if _HISTOGRAM_IMPORT_ERROR is not None:
            raise ImportError(
                "DropoutBooster requires sklearn histogram "
    ...
    def _validate_brat_d_params(self) -> None:
        if self.loss != "squared_error":
            raise ValueError(
                "DropoutBooster currently supports only "
                "loss='squared_error'."
            )
